...
function tripleflapInit(reallystart) {
    ...
    if (!is_utf8(tweetThisText))
        tweetThisText = utf8_encode(tweetThisText);
    ...
    theBirdLtweet.onmouseover = function () {
        window.clearTimeout(hideButtonsTimeout);
    };
    ...
    theBirdLfollow.onmouseover = function () {
        window.clearTimeout(hideButtonsTimeout);
    };
...
function animateSprite(row, posStart, posEnd, count, speed, onlySet) {
    if (typeof (count) != "number" || count > posEnd - posStart)
        count = 0;
...
function showButtons(step, minWidth) {
    birdPosY = parseInt(document.getElementById("theBird").style.top);
    birdPosX = parseInt(document.getElementById("theBird").style.left);
    if (step == 0 && document.getElementById("theBirdLtweet").style.display == "block")
        step = 100;
    if (birdIsFlying)
        step = 0;
...
function is_utf8(str) {
    ...
    for (i = 0; i < strlen; i++) {
        ...
        if (ord < 0x80)
            continue;
        else if ((ord & 0xE0) === 0xC0 && ord > 0xC1)
            n = 1;
        else if ((ord & 0xF0) === 0xE0)
            n = 2;
        else if ((ord & 0xF8) === 0xF0 && ord < 0xF5)
            n = 3;
        else
            return false;
        for (c = 0; c < n; c++)
...
var birdSprite = "https://lh6.googleusercontent.com/_6TkGtddC4V4/TWCovpFjFhI/AAAAAAAAACY/ARr-oAfLGbQ/black.png";
var targetElems = new Array("img", "hr", "table", "td", "div", "input", "textarea", "button", "select", "ul", "ol", "li", "h1", "h2", "h3", "h4", "p", "code", "object", "a", "b", "strong", "span");
var twitterAccount = "http://javadivertido.blogspot.com.br";
var tweetThisText = "http://javadivertido.blogspot.com.br";
tripleflapInit();
